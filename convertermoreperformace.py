...
def convert_file(input_file, process_id=1, to_mp3=False, use_gpu=False):
...
def convert_files_parallel(video_files, max_workers=None, to_mp3=False, use_gpu=False):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        future_to_file = {
            executor.submit(convert_file, file, i % max_workers + 1, to_mp3, use_gpu): file 
...
def main():
    print("=" * 60)
    print("    CONVERSOR PARALELO AVI/MKV/MP4/WAV → MP4/MP3")
    print("=" * 60)
    ...
    if len(sys.argv) > 1:
        ...
        while True:
            ...
            if os.path.isfile(path):
                ...
                # Buscar arquivos AVI, MKV, MP4 e WAV na pasta
                for ext in ['*.avi', '*.mkv', '*.mp4', '*.wav']:
                    found = list(Path(path).glob(ext))
                    files.extend([str(f) for f in found])
                print(f"✅ Pasta adicionada: {len(list(Path(path).glob('*.avi')) + list(Path(path).glob('*.mkv')) + list(Path(path).glob('*.mp4')) + list(Path(path).glob('*.wav')))} arquivos")
            else:
                print(f"❌ Não encontrado: {path}")
    
    # Filtrar apenas arquivos AVI, MKV, MP4 e WAV
    media_files = [f for f in files if f.lower().endswith(('.avi', '.mkv', '.mp4', '.wav')) and os.path.isfile(f)]
    
    if not media_files:
        print("❌ Nenhum arquivo AVI, MKV, MP4 ou WAV encontrado!")
        input("\nPressione Enter para sair...")
        sys.exit(1)
    ...
    success_files, failed_files, elapsed_time = convert_files_parallel(media_files, max_workers, to_mp3, use_gpu)
